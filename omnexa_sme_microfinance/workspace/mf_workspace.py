...
def _build_content(link_rows: list[dict], ws) -> str:
	...
	for row in link_rows:
		if row.get("type") == "Card Break":
			if section_idx:
				content.append({"id": f"mf-sp-{section_idx}", "type": "spacer", "data": {"col": 12}})
			content.append(
				{
	...
	if ws.number_cards:
		...
		for idx, nc in enumerate(ws.number_cards):
			content.append(
				{"id": f"mf-nc-{idx}", "type": "number_card", "data": {"number_card_name": nc.number_card_name, "col": 4}}
			)
	if ws.charts:
		content.append({"id": "mf-ch-h", "type": "header", "data": {"text": '<span class="h5"><b>📈 Charts</b></span>', "col": 12}})
		for idx, ch in enumerate(ws.charts):
			content.append(
				{"id": f"mf-ch-{idx}", "type": "chart", "data": {"chart_name": ch.label or ch.chart_name, "col": 4}}
			)
